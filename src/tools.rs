...
impl ReadBlackboardTool {
    pub fn new(service: Arc<BlackboardService>) -> Self { Self { service } }
}
impl Tool for ReadBlackboardTool {
    fn name(&self) -> &'static str { "read_blackboard" }
    fn description(&self) -> &'static str { "Reads the active blackboard state for a session" }
    fn input_schema(&self) -> Value {
        json!({
            "type": "object",
            "properties": {
    ...
    fn call(&self, args: &serde_json::Map<String, Value>) -> Result<Value, Value> {
        let session_id = args.get("session_id").and_then(Value::as_str).ok_or_else(|| json!({ "code": -32602, "message": "Missing session_id" }))?;
...
impl Tool for PatchBlackboardTool {
    fn name(&self) -> &'static str { "patch_blackboard" }
    fn description(&self) -> &'static str { "Merges a JSON patch into the active blackboard state" }
    fn input_schema(&self) -> Value {
        json!({
            "type": "object",
            "properties": {
    ...
    fn call(&self, args: &serde_json::Map<String, Value>) -> Result<Value, Value> {
        let session_id = args.get("session_id").and_then(Value::as_str).ok_or_else(|| json!({ "code": -32602, "message": "Missing session_id" }))?;
        let patch = args.get("json_patch").and_then(Value::as_object).ok_or_else(|| json!({ "code": -32602, "message": "Missing json_patch" }))?;
...
impl Tool for ArchiveSubtaskTool {
    fn name(&self) -> &'static str { "archive_subtask" }
    fn description(&self) -> &'static str { "Archives a specific key from the active blackboard to cold storage" }
    fn input_schema(&self) -> Value {
        json!({
            "type": "object",
            "properties": {
    ...
    fn call(&self, args: &serde_json::Map<String, Value>) -> Result<Value, Value> {
        let session_id = args.get("session_id").and_then(Value::as_str).ok_or_else(|| json!({ "code": -32602, "message": "Missing session_id" }))?;
        let key = args.get("key_to_archive").and_then(Value::as_str).ok_or_else(|| json!({ "code": -32602, "message": "Missing key_to_archive" }))?;
        match self.service.archive_subtask(session_id, key) {
            Ok(Ok(msg)) => Ok(json!({ "content": [{ "type": "text", "text": msg }] })),
            Ok(Err(msg)) => Ok(json!({ "content": [{ "type": "text", "text": msg }], "isError": true })),
            Err(e) => Err(json!({ "code": -32603, "message": e.to_string() })),
        }
    }
}
...
impl ToolRegistry {
    ...
    pub fn list_tools(&self) -> Value {
        let tools_json: Vec<Value> = self.tools.iter().map(|t| {
            json!({
                "name": t.name(),
    ...
    pub fn call_tool(&self, name: &str, args: &serde_json::Map<String, Value>) -> Result<Value, Value> {
